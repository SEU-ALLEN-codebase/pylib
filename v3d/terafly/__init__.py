...
@static_vars(loaded={})
def get_subvolume(path, x0, x1, y0, y1, z0, z1):
    """
    different from vaa3d, it returns the image of its original pixel size.
    it uses decorator to store metadata (the image manager instance), no need to load it each time.
    ...
    """
    if path not in get_subvolume.loaded:
        get_subvolume.loaded[path] = instance(path)
    return get_subvolume.loaded[path].load_subvolume(y0, y1, x0, x1, z0, z1)
